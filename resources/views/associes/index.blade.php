            ...
            <div class="card mb-4 mx-4">
                <div class="card-header pb-0">
                    <div class="d-flex flex-row justify-content-between">
                        <div>
                            <h5 class="title mb-0">Tous Associés</h5>
                        </div>
                        @can('ajouter-gerant')
                            <a style="background-color:#00FFFF" href="{{ route('associes.create') }}" class="btn btn-sm mb-0 btn-custom" type="button" >+&nbsp; Ajouter</a>
                        @endcan
                        ...
                        <table class="table table-striped align-items-center mb-0">
                            ...
                            <tbody>
                                ...
                                <tr  style="cursor: pointer;">                                        <td class="ps-4">
                                            <div class="text-xs font-weight-bold copy-id" data-copy="{{ $associé->id }}" data-bs-toggle="tooltip" data-bs-placement="top" title="Click to copy" style="cursor: pointer;">

                                            <p class="text-xs font-weight-bold mb-0">{{ $associé->id }}</p>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="avatar avatar-sm me-3" style="background-color: #{{ substr(md5($associé->name), 0, 6) }}; border-radius: 50%; display: flex; align-items: center; justify-content: center; width: 30px; height: 30px; margin-left:12px">
                                                <span style="color: white; font-size: 14px;">{{ strtoupper(substr($associé->name, 0, 1)) }}</span>
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            <div class="text-xs font-weight-bold copy-name" data-copy="{{ $associé->name }}" data-bs-toggle="tooltip" data-bs-placement="top" title="Click to copy" style="cursor: pointer;">

                                            <p class="text-xs font-weight-bold mb-0">{{ $associé->name }}</p>
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            <div class="text-xs font-weight-bold copy-email" data-copy="{{ $associé->email }}" data-bs-toggle="tooltip" data-bs-placement="top" title="Click to copy" style="cursor: pointer;">
                                                {{ $associé->email }}
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            @if ($associé->cin)
                                                <div class="text-xs font-weight-bold copy-cin" data-copy="{{ $associé->cin }}" data-bs-toggle="tooltip" data-bs-placement="top" title="Click to copy" style="cursor: pointer;">
                                                    {{ $associé->cin }}
                                                </div>
                                            @else
                                        ...
                                        <td class="text-center">
                                            <div class="text-xs font-weight-bold copy-role" data-copy="{{ $associé->role }}" data-bs-toggle="tooltip" data-bs-placement="top" title="Click to copy" style="cursor: pointer;">
                                                {{ $associé->role }}
                                            </div>
                                        </td>
                                        <td class="text-center ">
                                            @if ($associé->societe)
                                            <div class="text-xs font-weight-bold copy-societe" data-copy="{{ $associé->societe->name }}" data-bs-toggle="tooltip" data-bs-placement="top" title="Click to copy" style="cursor: pointer;">


                                                    {{ $associé->societe->name }}</div>
                                            @else
                                                <p class="text-xs font-weight-bold mb-0">---</p>
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-center ">
                                            <div class="d-flex  font-weight-bold">
                                                ...
                                                <a href="{{ route('associes.edit', $associé->id) }}" class="mx-3" data-bs-toggle="tooltip" data-bs-original-title="Edit gerant">
                                                    <i  class="fas fa-user-edit " style="color: #0eda8f"></i>
                                                </a>
                                                <form id="deleteForm_{{ $associé->id }}" action="{{ route('associes.destroy', $associé->id) }}" method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" onclick="showConfirmation('deleteForm_{{ $associé->id }}')" style="background: none; border: none; padding: 0; margin: 0; cursor: pointer;">
                                            ...
                                            <a href="{{ route('associes.show',$associé->id    )}}" class="mx-3" data-bs-toggle="tooltip" data-bs-original-title="Details société">
