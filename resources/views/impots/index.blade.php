            ...
            <div class="card mb-4 mx-4">
                <div class="card-header pb-0">
                    <div class="d-flex flex-row justify-content-between">
                        <div>
                            <h5 class="mb-0">Tous Impots</h5>
                        </div>
                        @can('manage-all-gerants')

                        <a class="btn btn-sm mb-0 btn-custom" href="{{ route('impots.create') }}"  type="button">+&nbsp; Ajouter</a>
                    @endcan
                    </div>
                </div>
                        ...
                        <table class="table table-striped align-items-center mb-0">
                            ...
                            <tbody>
                                ...
                                <tr  style="cursor: pointer;">                                        <td class="ps-4">
                                            <div class="text-xs font-weight-bold copy-login" data-login="{{ $impot->id }}" data-bs-toggle="tooltip" data-bs-placement="top" title="Click to copy" style="cursor: pointer;">

                                            <p class="text-xs font-weight-bold mb-0">{{ $impot->id }}</p>
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            <div  class="avatar avatar-sm me-3 mr-6" style="background-color: #{{ substr(md5($impot->login), 0, 6) }}; border-radius: 50%; display: flex; align-items: center; justify-content: center; width: 30px; height: 30px;margin-left:12px">
                                                <span class="text-xs font-weight-bold mb-0" style="color: white; font-size: 14px;">{{ strtoupper(substr($impot->login, 0, 1)) }}</span>
                                            </div>
                                        </td>
                                        <td class="text-center ">
                                            <div class=" font-weight-bold copy-cin"  style="cursor: pointer;">

                                            @if ($impot->societe)
                                                <p class="text-xs font-weight-bold mb-0 copy-cin">{{ $impot->societe->name }}</p>
                                            @else
                                        ...
                                        <td class="text-center">
                                            <div class="text-xs font-weight-bold copy-login" data-login="{{ $impot->login }}" data-bs-toggle="tooltip" data-bs-placement="top" title="Click to copy" style="cursor: pointer;">
                                                {{ $impot->login }}
                                            </div>
                                        </td>

                                        <td class="text-center">
                                            <div class="text-xs font-weight-bold copy-password" data-copy="{{ $impot->password }}" data-bs-toggle="tooltip" data-bs-placement="top" title="Click to copy" style="cursor: pointer;">
                                        ...
                                        <td class="text-center">
                                            <span class="text-secondary text-xs font-weight-bold">{{ $impot->code_acce }}</span>
                                        </td>
                                        <td class="text-center ">
                                       <div class="text-center d-flex font-weight-bold mb-0 ">
                                            @can('manage-all-gerants')
                                            <a href="{{ route('impots.edit', $impot->id) }}" class="mx-3" data-bs-toggle="tooltip" data-bs-original-title="Edit impot">
                                                <i  class="fas fa-user-edit " style="color: #0eda8f"></i>
                                            </a>
                                            
                                            <form id="deleteForm_{{ $impot->id }}" action="{{ route('impots.destroy', $impot->id) }}" method="post" onsubmit="return false;">
                                                @csrf
                                                @method('DELETE')
                                                <button onclick="showConfirmation('deleteForm_{{ $impot->id }}')" type="submit" style="background: none; border: none; padding: 0; margin: 0; cursor: pointer;">
                                                    <i style="color: red"  class="fas fa-trash-alt"></i>
                                                </button>
                                            </form>
                                           
                                        @endcan
                                        <a href="{{ route('impots.show',$impot->id)}}" class="mx-3" data-bs-toggle="tooltip" data-bs-original-title="Details impot">
...
<script>
    ...
    function showConfirmation(formId) {
        Swal.fire({
            title: 'Supprimer un Impot?',
            text: 'Le cas échéant, cet impot sera supprimé!',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                Swal.fire('supprimer', 'Le impot a été Supprimer', 'success');
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        var copyElements = document.querySelectorAll('.copy-login, .copy-password, .copy-societe');
